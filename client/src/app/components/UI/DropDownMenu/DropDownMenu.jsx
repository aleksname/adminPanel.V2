...
export default function DropDownMenu({ content, title, className, classNameDropDown, dropDownWrapper, onClick }) {
  ...
  return (
    <div className={`dropDownWrapper ${dropDownWrapper}`}>
      <button onClick={handleOpen} className={`dropDownMenu ${classNameDropDown}`}>
        <div>{title}</div>
